...
export default function CaseDetailPage() {
  ...
  if (!caseData && !loading) return (
    <div className="card" style={{marginTop: 50, textAlign: 'center', padding: 40}}>
      <div className="empty-state-icon"><span className="material-icons-outlined">error_outline</span></div>
      <div className="empty-state-title">{lang === 'kz' ? 'Іс табылмады' : 'Дело не найдено'}</div>
      <button className="btn btn-primary" onClick={() => navigate('/cases')}>{t('cases.title')}</button>
    </div>
  );
      ...
      <div className="card" style={{marginBottom:20}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px 24px', fontSize:'var(--font-sm)'}}>
          ...
          {caseData.incident_date && (
            <div>
              <span style={{color:'var(--text-muted)'}}>{lang === 'kz' ? 'Оқиға күні' : 'Дата происшествия'}:</span>
              <div style={{fontWeight:500, marginTop:2}}>{new Date(caseData.incident_date).toLocaleString()}</div>
            </div>
          )}
          {caseData.incident_place && (
            <div>
              <span style={{color:'var(--text-muted)'}}>{lang === 'kz' ? 'Оқиға орны' : 'Место происшествия'}:</span>
              <div style={{fontWeight:500, marginTop:2}}>{caseData.incident_place}</div>
            </div>
          )}
          {(caseData.description_ru || caseData.description_kz) && (
